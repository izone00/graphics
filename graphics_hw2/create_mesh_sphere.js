...
export function create_mesh_sphere(
  gl,
  SPHERE_DIV,
  loc_aPosition = 0,
  loc_aNormal = 1,
  ...
  for (j = 0; j <= SPHERE_DIV; j++) {
    ...
    for (i = 0; i <= SPHERE_DIV; i++) {
      ...
      positions.push(cos_theta * sin_phi * SPHERE_RADIUS); // x
      positions.push(sin_theta * sin_phi * SPHERE_RADIUS); // y
      positions.push(cos_phi * SPHERE_RADIUS); // z
  ...
  return new Mesh({
    gl,
    vao,
    draw_call: "drawElements",
    draw_mode: gl.TRIANGLES,
    n: indices.length,
    index_buffer_type: gl.UNSIGNED_SHORT,
    // loc_aPosition,
  });
}
